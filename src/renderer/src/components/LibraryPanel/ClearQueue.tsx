...
const ClearQueueIcon = styled.svg<{ isQueueEmpty: boolean }>`
  fill: ${({isQueueEmpty}) => isQueueEmpty ? '#444' : 'gray'};
  height: 35px;
  width: 35px;

  &:hover { fill: ${({isQueueEmpty}) => isQueueEmpty ? '#444' : 'white'}; }
  &:active { fill: ${({isQueueEmpty}) => isQueueEmpty ? '#444' : '#666'}; }
`;